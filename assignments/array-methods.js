...
let largeShirts = [];
largeShirts.push(runners.filter(function(currentValue) {
...
let emailList = [];
runners.forEach(function(currentValue) {
    return emailList.push(currentValue.email)
    });
console.log(emailList);
// Problem 2
//Uppercase all the company names
let companyCaps = [];
companyCaps = runners.map(function(currentValue){
    return currentValue.company_name.toUpperCase();
});
console.log(companyCaps);
// Problem 3
//How many people have small shirts?
let smallShirts = [];
smallShirts.push(runners.filter(function(currentValue) {
